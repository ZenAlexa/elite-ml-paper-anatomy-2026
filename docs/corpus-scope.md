...
`data/processed/analysis_sample.csv` 固定两个互不重叠的队列：

1. `foundation_200`：ICLR 100 篇、ICML 100 篇，包含两会全部 4 篇 Outstanding；
2. `replication_200`：从首批未入选论文中再抽取 ICLR 100 篇、ICML 100 篇。

调度器先完成首批 200，再开放复现批。两个队列采用完全相同的阅读协议和结构化变量。阶段性停止点分别为首批 200 全部完成、复现批 200 全部完成和合并 400 全部完成。760 篇资格总体继续保留，400 篇之外已完成的论文作为扩展证据，不改变分析分母。
